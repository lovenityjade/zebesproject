...
// Keep translated actions within their table cell; long labels wrap naturally.
void NextAction(const char* Label){
    const float Width=SMUI::CalcTextSize(Label).x+SMUI::GetStyle().FramePadding.x*2;
    const float End=SMUI::GetCursorScreenPos().x+SMUI::GetContentRegionAvail().x;
    if(SMUI::GetItemRectMax().x+SMUI::GetStyle().ItemSpacing.x+Width<=End)SMUI::SameLine();
}
bool Choose(const char* Id,FString& Current,const TArray<JV>& Values,bool Objects=false){
    ...
    bool Changed=false;
    if(SMUI::BeginCombo(Id,TCHAR_TO_UTF8(*Preview))){
        for(const auto& V:Values){FString Value=Objects?V->AsObject()->GetStringField(TEXT("value")):V->AsString();
            ...
            if(SMUI::Selectable(TCHAR_TO_UTF8(*Label),Value==Current)){Current=Value;Changed=true;}
        }SMUI::EndCombo();
    }return Changed;
}
...
bool FSMSystemMenu::DrawSeedCatalog(bool Busy){
    ...
    if(!Catalog || !Options || !Techniques || !Adjustments){SMUI::TextWrapped("Cannot read seed settings. Reload the slot before editing.");return false;}
    ...
    if(RandomPage==0 || All){
        ...
        if(Row("Settings string","Copy your complete randomizer configuration to share it. Import replaces the draft settings but keeps your seed number. It does not change an already generated save.")){
            ...
            SMUI::InputTextMultiline("##settings-string",SettingsString.GetData(),SettingsString.Num(),ImVec2(-1,72));
            if(SMUI::Button("Copy settings")){
                FString Text,Error;
                if(SMSeedSettings::ExportString(NextRequest(),Text,Error)){
                    FCStringAnsi::Strncpy(SettingsString.GetData(),TCHAR_TO_UTF8(*Text),SettingsString.Num());SMUI::SetClipboardText(SettingsString.GetData());Status=TEXT("Settings string copied. Share the seed number separately to reproduce the same world.");
                }else Status=Error;
            }
            NextAction("Paste");if(SMUI::Button("Paste")){
                const char* Text=SMUI::GetClipboardText();
            ...
            NextAction("Import settings");if(SMUI::Button("Import settings")){
        ...
        if(Row("Import / export configuration","Local JSON includes every seed option, technique, tolerance and tablet quota. Imports replace this draft; generated saves remain immutable.")){
            ...
            SMUI::InputText("##path",PresetPath,UE_ARRAY_COUNT(PresetPath));
            if(SMUI::Button("Export")){
            ...
            SMUI::SameLine();if(SMUI::Button("Import")){
    ...
    if(RandomPage==1 || All){
        ...
        if(Row("No Advanced Techs","Disable advanced traversal and heat runs in both generation and tracker logic. Basic wall jumps and full-length shinesparks remain enabled.")){Changed|=SMUI::Checkbox("##noadvanced",&NoAdvancedTechs);EndRow();}
        if(Row("Reset skill overrides","Return individual techniques and tolerances to the selected base preset.")){if(SMUI::Button("Use base preset")){Techniques=MakeShared<FJsonObject>();Adjustments=MakeShared<FJsonObject>();Changed=true;}EndRow();}
    }
    ...
    for(const auto& Entry:Catalog->GetArrayField(TEXT("fields"))){
        ...
        if((Type==TEXT("goals") || Type==TEXT("patches")) && RowUsesTable){SMUI::EndTable();RowUsesTable=false;SMUI::SetNextItemWidth(-1);}
        bool Edit=false;
        ...
        if(Type==TEXT("choice")){
            ...
            FString Multi;if(F->TryGetStringField(TEXT("multiple"),Multi) && Value==TEXT("random")){
                ...
                if(SMUI::TreeNode("Allowed choices")){for(auto& V:F->GetArrayField(TEXT("choices"))){auto C=V->AsObject();FString S=C->GetStringField(TEXT("value"));if(S==TEXT("random"))continue;bool Enabled=Contains(Allowed,S);if(SMUI::Checkbox(TCHAR_TO_UTF8(*C->GetStringField(TEXT("label"))),&Enabled)){if(Enabled)Allowed.Add(String(S));else Allowed.RemoveAll([&](const JV& X){return Text(X)==S;});Options->SetArrayField(Multi,Allowed);Edit=true;}}SMUI::TreePop();}
                if(Allowed.IsEmpty())SMUI::TextWrapped("Select at least one allowed choice.");
            }
            ...
            if(Mode==TEXT("fixed")){double N=Current->Type==EJson::Number?Current->AsNumber():FCString::Atod(*Text(Current));float V=float(N);if(SMUI::SliderFloat("##number",&V,F->GetNumberField(TEXT("minimum")),F->GetNumberField(TEXT("maximum")),F->GetBoolField(TEXT("decimal"))?"%.1f":"%.0f")){Options->SetNumberField(Key,F->GetBoolField(TEXT("decimal"))?FMath::RoundToDouble(V*10)/10:FMath::RoundToDouble(V));Edit=true;}}
            ...
            if(Mode==TEXT("Custom"))for(auto& P:F->GetArrayField(TEXT("patches"))){auto Patch=P->AsObject();FString Id=Patch->GetStringField(TEXT("id"));bool On=Contains(Selected,Id);if(SMUI::Checkbox(TCHAR_TO_UTF8(*Patch->GetStringField(TEXT("title"))),&On)){if(On)Selected.Add(String(Id));else Selected.RemoveAll([&](const JV& V){return Text(V)==Id;});Options->SetArrayField(Custom,Selected);if(Selected.IsEmpty())Options->SetStringField(Key,TEXT("off"));Edit=true;}if(SMUI::IsItemHovered())SMUI::SetTooltip("%s",TCHAR_TO_UTF8(*Patch->GetStringField(TEXT("description"))));}
        }else if(Type==TEXT("goals")){
            auto Selected=Array(Current);SMUI::Text("%d selected",Selected.Num());
            if(MenuPreviewFrame && MenuPreviewName.Contains(TEXT("-goals")))SMUI::SetNextItemOpen(true,ImGuiCond_Once);
            if(SMUI::TreeNode("Choose goals")){
                ...
                for(const auto& Group:Groups){if(MenuPreviewFrame && MenuPreviewName.Contains(TEXT("-goals")))SMUI::SetNextItemOpen(true,ImGuiCond_Once);if(SMUI::TreeNode(TCHAR_TO_UTF8(*Group))){for(auto& G:Catalog->GetArrayField(TEXT("objectives"))){auto Goal=G->AsObject();FString Name=Goal->GetStringField(TEXT("name"));if(Goal->GetStringField(TEXT("category"))!=Group || !Goal->GetBoolField(TEXT("available")) || (Name==TEXT("nothing") && Key!=TEXT("objective")) || Name==TEXT("finish scavenger hunt"))continue;
                    bool On=Contains(Selected,Name);bool Conflict=Key==TEXT("objective") && !On && GoalConflict(Catalog,Selected,Goal);SMUI::BeginDisabled(!On && Key==TEXT("objective") && (Selected.Num()>=18 || Conflict));
                    SMUI::PushID(TCHAR_TO_UTF8(*Name));if(SMUI::Checkbox("##goal",&On)){if(On)Selected.Add(String(Name));else Selected.RemoveAll([&](const JV& V){return Text(V)==Name;});Options->SetArrayField(Key,Selected);Edit=true;}SMUI::SameLine();SMUI::TextWrapped("%s",TCHAR_TO_UTF8(*Name));SMUI::EndDisabled();if(Conflict && SMUI::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled))SMUI::SetTooltip("Conflicts with a selected objective.");SMUI::PopID();
                }SMUI::TreePop();}}
                SMUI::TreePop();
            }if(Selected.IsEmpty())SMUI::TextWrapped("Select at least one goal.");
        }
        if(Options->HasField(Key) && SMUI::SmallButton("Reset to default")){Options->RemoveField(Key);Edit=true;}
        SMUI::EndDisabled();Changed|=Edit;EndRow();
    }
    ...
    if(RandomPage==7 || All){
        ...
        if(!All){FString Selected=GroupNames[TechniquePage]->AsString();SMUI::SetNextItemWidth(-1);if(Choose("##tech-category",Selected,GroupNames))for(int I=0;I<GroupNames.Num();I++)if(Text(GroupNames[I])==Selected)TechniquePage=I;}
        if(NoAdvancedTechs)SMUI::TextWrapped("No Advanced Techs overrides these values during generation. Your custom choices are preserved for when you turn it off.");
        for(auto& T:Catalog->GetArrayField(TEXT("techniques"))){auto Tech=T->AsObject();FString Key=Tech->GetStringField(TEXT("key"));
            ...
            SMUI::BeginDisabled(NoAdvancedTechs);bool Edit=SMUI::Checkbox("Allowed in logic",&Enabled);
            const int Values[]={0,1,5,10,25,50,100,200,400,800};int Index=0;for(int I=0;I<10;I++)if(Difficulty>=Values[I])Index=I;
            if(SMUI::Combo("##difficulty",&Index,"Unrated\0Easy\0Medium\0Hard\0Very hard\0Hardcore\0Mania\0God\0Samus\0Impossible\0")){Difficulty=Values[Index];Edit=true;}
            ...
            if(Techniques->HasField(Key) && SMUI::SmallButton("Use preset value")){Techniques->RemoveField(Key);Changed=true;}SMUI::EndDisabled();EndRow();
        }
    }
