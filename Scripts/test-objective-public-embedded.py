...
lib=root/'objectives-map/libsm_native.so';native=C.CDLL(str(lib))
generate=native.sm_randomizer_generate
generate.argtypes=[C.c_char_p,C.c_char_p,C.c_char_p,C.c_int]
reports=[]
for file in sorted((root/'objective-public-seeds').glob('seed-*.json')):
    ...
    print('OBJECTIVE_PUBLIC_EMBEDDED_PASS',actual['seed'],flush=True)
assert len(reports)==9
(root/'objectives-map/public-embedded.json').write_text(json.dumps(dict(passed=True,seeds=reports,
