...
source=source.replace("out=root/'tracker-results'", "out=root/'fast-tourian/tracker'")
source=source.replace("root/'Native/build/libsm_native.so'","root/'fast-tourian/libsm_native.so'")
exec(compile(source,'objective-tracker-fixture','exec'))
reports=[]
for path in sorted((root/'fast-tourian/public').glob('seed-*.json')):
 ...
 print('FAST_TOURIAN_TRACKER_PASS',m['seed'],len(cases),flush=True)
