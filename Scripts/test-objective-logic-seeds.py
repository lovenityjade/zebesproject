#!/usr/bin/env python3
"""Fresh generated pools with real objective-aware solver routes, gaming-pc only.

The source generator override is confined to this fixture. Public nondefault
goal guards remain until original native UI and dependencies are complete.
"""
import hashlib,json,os,subprocess,sys
from pathlib import Path
root=Path(sys.argv[1]).resolve()
assert os.uname().nodename=='gaming-pc' and (root/'ISOLATED_TEST_DIRECTORY').is_file()
out=root/'objective-logic-seeds';out.mkdir(exist_ok=True)
cases=[
 dict(seed=15092900,options={},goals=['kill all G4'],required='off'),
 dict(seed=15092901,options={'majorsSplit':'FullWithHUD','areaRandomization':'full'},
      goals=['tickle the red fish','collect 25% items','explore 50% map'],required=2,hidden=True),
 dict(seed=15092902,options={'majorsSplit':'Chozo'},
      goals=['clear green brinstar','visit the animals','activate chozo robots'],required='off'),
 dict(seed=15092903,options={'bossRandomization':'on','doorsColorsRando':'on'},
      goals=['collect all upgrades','kill all beetoms'],required='off'),
 dict(seed=15092904,options={},
      goals=['nothing'],required=1),
 dict(seed=15092905,options={'minorQty':50},goals=['collect 100% items'],required='off'),
 dict(seed=15092906,options={},goals=['tickle the red fish','activate chozo robots','visit the animals'],required=1)]
if len(sys.argv)==2:
 reports=[]
 for i,case in enumerate(cases):
  ...
  print('OBJECTIVE_LOGIC_SEED_PASS',i,m['seed'],obj['names'],flush=True)
 (out/'verification.json').write_text(json.dumps(dict(seeds=reports,
   modules={name:hashlib.sha256((root/'Randomizer'/name).read_bytes()).hexdigest()
            for name in ('sm_objectives.py','sm_validation.py','sm_tracker_data.py')},
...
def resolve(request):
 rules=original(request)
 rules['options'].update(objective=case['goals'],nbObjectivesRequired=case['required'],
                         hiddenObjectives='on' if case.get('hidden') else 'off')
 if case.get('hidden'):
  rules['options'].update(objectiveRandom='true',nbObjective=len(case['goals']),objectiveMultiSelect=case['goals'])
 return rules
sm_options.resolve=resolve
request=dict(seed=case['seed'],skill='regular',options=case['options'])
...
if not result.get('ok'):
 print('OBJECTIVE_LOGIC_CANDIDATE_REJECTED',i,attempt,result,flush=True)
 raise SystemExit(0)
...
assert set(obj['names'])==set(expected),(obj,expected)
assert proof['names']==obj['names'] and proof['required']==obj['required']
assert obj['required']==(len(expected) if case['required']=='off' else case['required'])
