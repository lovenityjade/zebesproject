...
fixture=fixture.replace("out=root/'tracker-results'","out=root/'gameplay/public'").replace("root/'Native/build/libsm_native.so'","root/'gameplay/libsm_native.so'")
...
reports=[]
keys=[('nerfedCharge','nerfed-charge-v1'),('relaxed_round_robin_cf','round-robin-cf-v1'),('rando_speed','momentum-landing-v1')]
for i in range(2):
 options={key:'on' if i==0 or key=='nerfedCharge' else 'off' for key,_ in keys}
 q=dict(seed=15093600+i,skill='regular',options=options)
 ...
 assert RomPatches.NerfedCharge in m['logicPatches']
 assert (RomPatches.RoundRobinCF in m['logicPatches'])==(i==0)
 ...
 print('GAMEPLAY_SEED_TRACKER_PASS',m['seed'],len(steps),flush=True)
