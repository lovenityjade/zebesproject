#!/usr/bin/env python3
"""Every item-check step from seven objective-aware routes, embedded oracle."""
import os,sys
from pathlib import Path
root=Path(sys.argv[1]).resolve()
assert os.uname().nodename=='gaming-pc' and (root/'ISOLATED_TEST_DIRECTORY').is_file()
source=(root/'test-live-tracker-logic.py').read_text().split('v=evaluate(')[0]
source=source.replace("out=root/'tracker-results'", "out=root/'objectives-logic/tracker'")
...
reports=[]
for path in sorted((root/'objective-logic-seeds').glob('seed-*.json')):
 ...
 print('OBJECTIVE_LOGIC_TRACKER_PASS',m['seed'],len(cases),flush=True)
