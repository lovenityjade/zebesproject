#!/usr/bin/env python3
"""Render generated Scavenger worlds, original HUD and objective pause navigation."""
import hashlib,json,os,shutil,subprocess,sys,time
from pathlib import Path
root=Path(sys.argv[1]).resolve()
assert os.uname().nodename=='gaming-pc' and (root/'ISOLATED_TEST_DIRECTORY').is_file()
assert subprocess.run(['pgrep','-x','SMUnreal'],stdout=subprocess.DEVNULL).returncode==1
out=root/'scavenger';lib=root/'Native/build/libsm_native.so'
shutil.copy2(out/'libsm_native.so',lib.with_suffix('.so.next'));os.replace(lib.with_suffix('.so.next'),lib)
results=[]
for case in [0,1,2]:
    m=json.loads((root/f'scavenger/public/seed-{case:02d}.json').read_text())['manifest'];obj=m['nativeContext']['objectives']
    args=['./SMUnreal/Binaries/Linux/SMUnreal','/Engine/Maps/Entry','-SMObjectiveLogicTest','-SMScavengerTest',f'-SMStartTest={case}',
...
print('SCAVENGER_VULKAN_PASS',flush=True)
