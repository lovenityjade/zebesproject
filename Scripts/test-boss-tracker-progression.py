#!/usr/bin/env python3
"""Every check in four generated boss-seed progression logs, embedded oracle.
...
source=source.replace("out=root/'tracker-results'", "out=root/'connections/tracker'")
exec(compile(source,'boss-tracker-fixture','exec'))
reports=[]
for path in sorted((root/'boss-seeds').glob('seed-*.json')):
 m=json.loads(path.read_text())['manifest'];t=m['tracker'];visited=set();cases=[]
 assert t['topology']['mode']=='boss'
 for p in m['solverVerification']['progressionLog']:
  ...
  assert r['topologyMode']=='boss' and r['bossConnections']==t['topology']['bossPairs']
 ...
 print('BOSS_TRACKER_PROGRESSION_PASS',m['seed'],len(cases),flush=True)
