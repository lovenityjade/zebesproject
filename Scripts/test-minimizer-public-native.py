...
for slot in range(3):
 manifest=json.loads((root/f'minimizer/public/seed-{slot:02d}.json').read_text())['manifest']
 mini=manifest['nativeContext']['topology']['nativeMinimizer'];obj=manifest['nativeContext']['objectives']
 hunt=manifest['nativeContext'].get('scavenger')
...
# Remaining actual source death seams, with controlled native enemy state.
clear();put(0x79f,2)
for i in range(32):put(0xf78+i*64,1)
put(0x800e,5);put(0x7836,0);put(0xfb2,0)
routine('Ridley_Func_64',None)();assert read(0xd82a,1)[0]&1 and word(0xfa8)==0xc5a8
clear();put(0x79f,4);put(0xe54,0);put(0xf7a,256);put(0xf7e,480);put(0xfa4,1)
routine('Draygon_Func_31',None)();assert read(0xd82c,1)[0]&1 and word(0xfa8)==0x9294
clear();put(0x79f,3);put(0xe54,0);put(0xf78,0xe4bf);put(0xf8c,0);put(0x18a6,0);put(0xc18,0x100);put(0xc2c,0);put(0xfb2,0xd000)
routine('Phantoon_Shot',None)();assert read(0xd82b,1)[0]&1
# Original room-save PLM remains present in a mixed-domain world.
clear();put(0x79b,51958);put(0x7bb,51976);put(0x7a5,80)
C.memset(ram+0x1c37,0,80)
routine('sm_connections_room',None)()
assert any(word(0x1c37+i*2)==0xc842 for i in range(40))
...
(root/'minimizer/public-native-results.json').write_text(json.dumps(dict(passed=True,arrivals=arrivals,retainedChecks=counts,filteredMapCounts=True,nativeBossDeathSeams=4,bossSavePlm=True,rollback=True,bankSwitches=4,restoration=True,cpuOpcodes=0,nativeSha256=hashlib.sha256((root/'minimizer/libsm_native.so').read_bytes()).hexdigest()),indent=2)+'\n')
