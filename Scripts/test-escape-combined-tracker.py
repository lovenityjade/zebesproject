#!/usr/bin/env python3
"""Escape and Disabled Tourian progression through the embedded tracker."""
import os,sys
from pathlib import Path
root=Path(sys.argv[1]).resolve()
assert os.uname().nodename=='gaming-pc' and (root/'ISOLATED_TEST_DIRECTORY').is_file()
source=(root/'test-live-tracker-logic.py').read_text().split('v=evaluate(')[0]
source=source.replace("out=root/'tracker-results'", "out=root/'escape/combined-tracker'")
source=source.replace("root/'Native/build/libsm_native.so'","root/'escape/libsm_native.so'")
exec(compile(source,'objective-tracker-fixture','exec'))
reports=[]
for path in sorted((root/'escape/public').glob('seed-03.json')):
 ...
 for p in m['solverVerification']['progressionLog']:
  ...
  assert [c['enabled'] for c in r['checks']]==list(map(bool,t['topology'].get('nativeMinimizer',{}).get('checks',[1]*100)))
 ...
 assert len(cases)==sum(t['topology'].get('nativeMinimizer',{}).get('checks',[1]*100))
 reports.append(dict(seed=m['seed'],fingerprint=m['sha256'],cases=cases))
 print('ESCAPE_TRACKER_PASS',m['seed'],len(cases),flush=True)
