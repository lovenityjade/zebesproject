#!/usr/bin/env python3
"""Every check in four generated door-color seed progression logs, embedded oracle.
...
source=source.replace("out=root/'tracker-results'", "out=root/'door-colors/tracker'")
exec(compile(source,'boss-tracker-fixture','exec'))
reports=[]
for path in sorted((root/'door-color-seeds').glob('seed-*.json')):
 m=json.loads(path.read_text())['manifest'];t=m['tracker'];visited=set();cases=[]
 assert 'doorColors' in t['topology']
 ...
 print('DOOR_TRACKER_PROGRESSION_PASS',m['seed'],len(cases),flush=True)
