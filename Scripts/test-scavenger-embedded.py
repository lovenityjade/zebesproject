...
lib=root/'scavenger/libsm_native.so';native=C.CDLL(str(lib))
generate=native.sm_randomizer_generate
generate.argtypes=[C.c_char_p,C.c_char_p,C.c_char_p,C.c_int]
reports=[]
for file in sorted((root/'scavenger/public').glob('seed-*.json')):
    ...
    print('SCAVENGER_EMBEDDED_PASS',actual['seed'],flush=True)
assert len(reports)==3
(root/'scavenger/public-embedded.json').write_text(json.dumps(dict(passed=True,seeds=reports,
