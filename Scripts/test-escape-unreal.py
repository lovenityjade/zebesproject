#!/usr/bin/env python3
"""Render Disabled Tourian with the embedded oracle and original objective pause."""
import hashlib,json,os,shutil,subprocess,sys,time
from pathlib import Path
root=Path(sys.argv[1]).resolve()
assert os.uname().nodename=='gaming-pc' and (root/'ISOLATED_TEST_DIRECTORY').is_file()
assert subprocess.run(['pgrep','-x','SMUnreal'],stdout=subprocess.DEVNULL).returncode==1
out=root/'escape';lib=root/'Native/build/libsm_native.so'
shutil.copy2(out/'libsm_native.so',lib.with_suffix('.so.next'));os.replace(lib.with_suffix('.so.next'),lib)
results=[]
for case in [1]:
    m=json.loads((root/f'escape/integration/seed-{case:02d}.json').read_text())['manifest'];obj=m['nativeContext']['objectives']
    args=['./SMUnreal/Binaries/Linux/SMUnreal','/Engine/Maps/Entry','-SMObjectiveLogicTest','-SMEscapeTest',f'-SMStartTest={case}',
...
print('ESCAPE_VULKAN_PASS',flush=True)
