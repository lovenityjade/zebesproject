#!/usr/bin/env python3
"""Every check in six generated area/start seed progression logs, embedded oracle.
...
source=source.replace("out=root/'tracker-results'", "out=root/'areas/tracker'")
source=source.replace("root/'Native/build/libsm_native.so'","root/'areas/libsm_native.so'")
exec(compile(source,'area-tracker-fixture','exec'))
reports=[]
for path in sorted((root/'area-seeds').glob('seed-*.json')):
 ...
 print('AREA_TRACKER_PROGRESSION_PASS',m['seed'],len(cases),flush=True)
